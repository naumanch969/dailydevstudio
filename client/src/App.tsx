    ...
    <div className="min-h-screen ">
      {!isButtonHover && (
        <>
          <div
            ref={cursorDotRef}
            className="fixed top-0 left-0 tranform translate-x-[-50%] z-[100] translate-y-[-50%] rounded-full opacity-40 pointer-events-none w-[5px] h-[5px] text-white "
          ></div>
          <div
            ref={cursorOutlineRef}
            style={{ background: "rgba(175, 175, 175)" }}
            className="fixed top-0 left-0 tranform translate-x-[-50%] translate-y-[-50%] rounded-full z-[99] opacity-60 pointer-events-none w-[25px] h-[25px] cursor-outline"
          ></div>
        </>
      )}
